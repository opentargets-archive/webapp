angular.module('plugins')
    .directive('genomeBrowser', ['$log', 'cttvAPIservice', 'cttvUtils', function ($log, cttvAPIservice, cttvUtils) {
        ...
        return {
            ...
            + '<p>Gene Location: <a target=_blank href="http://www.ensembl.org/Homo_sapiens/Location/View?db=core;g={{target.ensembl_gene_id}}">Human {{target.chromosome}}:{{target.gene_start}}-{{target.gene_end}}</a></p><png filename="{{target.approved_symbol}}-browser.png"></png>',
            scope: {
                target: '=',
                disease: '=',
                width: "="
