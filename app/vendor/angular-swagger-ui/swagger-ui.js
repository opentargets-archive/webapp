...
angular
	...
	.service('swaggerParser', ["$q", "$sce", "$location", "swaggerModel", "swaggerTranslator", function($q, $sce, $location, swaggerModel, swaggerTranslator) {
		...
		function parseInfos(swagger, url, infos, defaultContentType) {
            ...
            infos.description = trustHtml( marked.parse( infos.description ) ); /* marked.js must be loaded gobally */
		...
		function parseOperations(swagger, resources, form, map, defaultContentType, openPath) {
			...
			for (path in swagger.paths) {
				...
				for (httpMethod in pathObject) {
					...
					operation.description = trustHtml( marked.parse( operation.description ) );
                    ...
                    for(var i=0; i<operation.tags.length; i++){
    					...
    					resource = resources[map[operation.tags[i]]];
		...
		function parseParameters(swagger, operation, pathParameters, form, defaultContentType) {
			...
			for (i = 0, l = parameters.length; i < l; i++) {
				...
				param.description = trustHtml( marked.parse( param.description ) );
		...
		function parseResponses(swagger, operation) {
			...
			if (operation.responses) {
				for (code in operation.responses) {
					...
					response.description = trustHtml( marked.parse( response.description ) );
					if (response.schema) {
